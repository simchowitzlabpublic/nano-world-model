# adapted from: https://github.com/ragor114/PyTorch-Frechet-Video-Distance
"""
DISCLAIMER
...
# Download file from given URL, cache to local directory (default ./loaded_models), and return as binary stream
...
# Load pretrained feature extraction model (usually I3D) from specified URL and place on specified device
...
def get_feature_detector(detector_url, device):
    ...
    if key not in _feature_detector_cache:
        # If detector_url points to a local file, load directly
        if os.path.exists(detector_url):
            _feature_detector_cache[key] = torch.jit.load(detector_url).eval().to(device)
        else:
...
"""
# Extract feature vectors for each video using pretrained feature extractor, then compute mean and covariance
# max_items limits number of videos to process, batch_size is number of videos processed at once
def compute_feature_stats(data, detector_url, detector_kwargs, batch_size, max_items, device, return_raw_features=False):
...
"""
# Input video tensor shape should be [num_videos, channels, num_frames, width, height]
def compute_fvd(y_true: torch.Tensor, y_pred: torch.Tensor, max_items: int, device: torch.device, batch_size: int, return_raw_features: bool = False, i3d_model_path: str = None):
    # URL from StyleGAN-V repository!
    detector_url = i3d_model_path
    detector_kwargs = dict(rescale=False, resize=True, return_features=True) # Return raw features before the softmax layer.
    if return_raw_features:
        raw_features_true = compute_feature_stats(y_true, detector_url, detector_kwargs, batch_size, max_items, device, return_raw_features=True)
        raw_features_pred = compute_feature_stats(y_pred, detector_url, detector_kwargs, batch_size, max_items, device, return_raw_features=True)
        ...
        mu_true, sigma_true = compute_feature_stats(y_true, detector_url, detector_kwargs, batch_size, max_items, device, return_raw_features=False)
        mu_pred, sigma_pred = compute_feature_stats(y_pred, detector_url, detector_kwargs, batch_size, max_items, device, return_raw_features=False)
